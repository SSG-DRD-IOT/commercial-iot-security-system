...
while(True):
    ...
    kernelKill = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (10,10))
    kernelBirth = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5))
    # kernel = disk
    # threshold = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, kernel)

    thresh = cv2.erode(thresh, kernelKill, iterations = 2) # iter = 2
    thresh = cv2.dilate(thresh, kernelBirth, iterations= 4) # iter = 3
